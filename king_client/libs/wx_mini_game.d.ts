...
declare const wx: {
    ...
    navigateToMiniProgram(object: {appId:string,path?:string,extraData?:any,envVersion?:string,success?:(res:any)=>void,fail?:(res:any)=>void,cmplete?:(res:any)=>void})
    /**
     ...
     */
    createRewardedVideoAd(object: {adUnitId: string}): RewardedVideoAd;
    /**
     * 触发分包加载
     */
    loadSubpackage(object: {name:string,success:(res:any)=>void,fail:(res:any)=>void,complete:(res:any)=>void}):LoadSubpackageTask;
}
...
declare interface RewardedVideoAd {
     ...
     * 监听激励视频广告加载事件
     */
    onLoad(callback: (res) => void);
    /**
     * 取消监听激励视频广告加载事件
     */
    offLoad(callback: (res) => void);
    /**
     * 监听激励视频错误事件
     */
     ...
     * 取消监听激励视频错误事件
     */
    offError(callback: (res) => void);
    /**
     * 监听用户点击 关闭广告 按钮的事件
     * @param res isEnded	boolean	视频是否是在用户完整观看的情况下被关闭的	>= 2.1.0
     */
    onClose(callback: (res) => void);
    /**
     * 取消监听用户点击 关闭广告 按钮的事件
     */
...
declare interface LoadSubpackageTask {
    onProgressUpdate(callback: (res) => void);
}

declare const canvas:{
    toTempFilePathSync(object:any):string;
}
...
declare interface DownloadTask {
    abort(): void;
    onProgressUpdate(callback: (res)=>void): void;
}
