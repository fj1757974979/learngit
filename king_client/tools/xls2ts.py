...
def get_dst_path(device):
    if device == "mobile":
        return "device_modules/mobile/data/"
    else:
        return "device_modules/pc/data/"
...
def gen_header(cls_name):
    if cls_name != "text":
        head = "///<reference path=\"text.d.ts\" />\n"
    else:
        head = ""
    return """// Generated by xls2ts.py
// DO NOT EDIT!
""" + head + """
declare module Data {
    class %s { """ % cls_name
...
def sheet2ts(device, sheet_data, cls_name):
    ...
    tsfilename = get_dst_path(device) + cls_name + ".d.ts"
    ...
    f.write("\n\t\t}\n\n\t\t")
    f.write("private static _keys = %s;\n" % _ids)
    f.write(gen_tail(cls_name))
    f.close()
