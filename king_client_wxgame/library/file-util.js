...
export const fs = {
    ...
    existsSync: (p, v) => {
        ...
        if (v != undefined) {
          ...
          let hasV = true;
          try {
            wxFs.accessSync(WX_ROOT + vp);
          } catch (e) {
            hasV = false;
          }
          if (hasV) {
            let ver = wxFs.readFileSync(WX_ROOT + vp, "utf8");
            if (ver != v) {
              fs_cache[p] = 0;
            } else {
              fs_cache[p] = 1;
		...
		//let time2 = Date.now();
		//console.log("check version consume: ", time2 - time1, "ms");
        ...
        if (cache == 0) {
            ...
            try {
				...
				//let time4 = Date.now();
				//console.log("check file consume: ", time4 - time3, "ms");
                return true;
            } catch (e) {
                p = path.normailze(p);
                fs_cache[p] = 0;
