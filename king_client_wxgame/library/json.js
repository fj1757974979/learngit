            ...
            if (path.isRemotePath(xhrURL)) { //判断是本地加载还是网络加载
                if (needCache(root, url)) {
                    ...
                    if (fs.existsSync(targetFilename, ver)) {
                        ...
                        let data = JSON.parse(fs.readSync(targetFilename));
                        resolve(data);
                    } else {
                        //通过url加载，加载成功后加入本地缓存
                      //console.log("ImageProcessor onLoadStart root:", root, "url:", url, "imageSrc:", xhrURL, " --> ", targetFilename, " version:", ver);
...
RES.processor.map("json", processor);
