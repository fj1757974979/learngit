...
--[[	
	...
	function decode (js_string)
		...
		-- 取下一个标记，但不移动游标
		local function fetch_next_token(tok)
			local b = js_string:byte(pos)
			local t = tok[b]
			return t
		end
		...
		local function read_string (tok)
			...
			until t == true
			local ret = js_string:sub(start-1, pos-1)
			local returnString = {}
			if string.find(ret, "\\u") then
				-- 兼容一下unicode“形式”的字符串
				local slen = string.len(ret)
				for i=1,slen do
					local t = string.byte(ret, i)
					local nt = string.byte(ret, i + 1)
					if t ~= 92 or (nt and nt ~= 117) then
						table.insert(returnString, t)
					end
				end
				returnString = map(string.char, returnString, ipairs)
				ret = table.concat(returnString)
			end
			return (base.loadstring("return " .. ret) ())
		...
		function read_array (o,i)
			...
			while true do
				-- 空数组监测
				local t = fetch_next_token(tt_array_value)
				if t == true then
					pos = pos + 1
					return o
				end
